...
int main(int argc, char *argv[]) {
    ...
    int timeout_value = 1000; // set timeout to 1 second
    setsockopt(internet_socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout_value, sizeof timeout_value); // set receive timeout
    ...
    while (1) {
        ...
        // process the received set of numbers
        int received_value;
        int local_max_value = -2147483647;
        char *token = strtok(buffer, ",");
        while (token != NULL) {
            received_value = atoi(token);
            printf("Last received value: %d\n", received_value);
            if (received_value > local_max_value) {
                local_max_value = received_value;
            }
            token = strtok(NULL, ",");
        }
